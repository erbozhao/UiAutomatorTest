...
//            sleep(TIMEOUT_MEDIUM);
    ...
    private fun testUIPressureOld() {
        ...
        sleep(TIMEOUT_VERY_SHORT.toLong())
        val seekBar = waitUiObject2sByClazz("android.widget.SeekBar", TIMEOUT_MEDIUM.toLong())[0]
        swip(seekBar, "left")
        sleep(TIMEOUT_VERY_SHORT.toLong())
        back()
        sleep(TIMEOUT_VERY_SHORT.toLong())
        ...
        sleep(TIMEOUT_VERY_SHORT.toLong())
        val homeSwitch = waitUiObject2sByClazz("android.widget.Switch", TIMEOUT_MEDIUM.toLong())[0]
        ...
        if (cleanPhoenix != null && cleanPhoenix.isEnabled) {
            ...
            getUiObject2s("android.widget.ImageView", true, 0.0, 0.2, 0.0, 0.2, 0.0, 0.3, 0.02, 0.3)[0].click()
        }
        // 设置默认浏览器
        ...
        sleep(TIMEOUT_SHORT.toLong())
        getUiObject2s("android.widget.ImageView", true, 0.5, 1.0, 0.01, 0.5, 0.0, 1.0, 0.7, 1.0)[0].click()
        sleep(TIMEOUT_VERY_SHORT.toLong())
        skipAppDialog()
    ...
    private fun testUIPressureNovel() {
        ...
        getUiObject2ByText("Me")?.click()
        sleep(TIMEOUT_MEDIUM.toLong())
        getUiObject2ByRes("com.google.android.gms:id/continue_button")?.let {
            it.click()
            sleep(TIMEOUT_MEDIUM.toLong())
        }
        getUiObject2ByText("Notifications")?.click()
        sleep(TIMEOUT_MEDIUM.toLong())
        back()
        sleep(TIMEOUT_VERY_SHORT.toLong())
        getUiObject2ByText("Library")?.click()
        sleep(TIMEOUT_MEDIUM.toLong())
        click(width * 1 / 5, height * 1 / 6)
        sleep(TIMEOUT_MEDIUM.toLong())
        repeat(20) {
            swip(0.7, 0.5, 0.3, 0.5)
            sleep(TIMEOUT_SHORT.toLong())
