...
public class WeatherGoBack extends PerCommon {
    private File resultFolder = new File(perFolder, "memoryleak");
    private File resultFile = new File(resultFolder, "memoryleak_80_weather-goback.txt");
    @Before
    ...
    public void testWeatherGoBack() {
        ...
        try {
            ...
            memoryThread.start();
            DumpheapThread startDumpheapThread = new DumpheapThread(BROWSER_PHX, device, model + "_Weather-GoBack-Start", resultFolder);
            ...
            // 测试场景: 点击天气20次
            for (int i = 0; i < 20; i++) {
                List<UiObject2> locations = getUiObject2s("android.widget.LinearLayout", true, 0, 0.4, 0, 0.2, 0, 0.4, 0.02, 0.2);
                if (locations == null || locations.size() == 0) {
                    sleep(TIMEOUT_LONG);
                    locations = getUiObject2s("android.widget.LinearLayout", true, 0, 0.4, 0, 0.2, 0, 0.4, 0.02, 0.2);
                }
            ...
            memoryThread.join();
            DumpheapThread endDumpheapThread = new DumpheapThread(BROWSER_PHX, device, model + "_Weather-GoBack-End", resultFolder);
            ...
            e.printStackTrace();
            screenshot(resultFolder + "/Img_" + model + "_Weather-GoBack-Exception_" + BROWSER_PHX + "_" + CommonUtil.getCurTimeForFile() + ".jpg");
        }
