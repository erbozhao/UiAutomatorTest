...
public class BrowseOneWindowDetailPage extends PerCommon {

    private File resultFolder = new File(perFolder, "memoryleak");
    private File resultFile = new File(resultFolder, "memoryleak_48_browseonewindow-detailpage.txt");
    ...
    @Test
    public void testBrowseOneWindowDetailPage() {
        // 获取设备名
        ...
        try {
            ...
            memoryThread.start();
            DumpheapThread startDumpheapThread = new DumpheapThread(BROWSER_PHX, device, model + "_BrowseOneWindow-DetailPage-Start", resultFolder);
            ...
            // 测试场景: 单窗口连续浏览网页20次
            for (int i = 0; i < 20; i++) {
                ShellCommand.execCmdByUiDevice(device, "am start -a android.intent.action.VIEW -d phxbrowser://www.baidu.com");
                sleep(5000);
            ...
            memoryThread.join();
            DumpheapThread endDumpheapThread = new DumpheapThread(BROWSER_PHX, device, model + "_BrowseOneWindow-DetailPage-End", resultFolder);
            ...
            e.printStackTrace();
            screenshot(resultFolder + "/Img_" + model + "_BrowseOneWindow-DetailPage-Exception_" + BROWSER_PHX + "_" + CommonUtil.getCurTimeForFile() + ".jpg");
        }
