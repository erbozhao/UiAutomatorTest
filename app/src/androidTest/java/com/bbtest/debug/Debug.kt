...
//            }
    ...
    private fun getRootUiObject2(rootContent: UiObject2): UiObject2? {
        ...
        for (children in childrens) {
            ...
            if (curWidth == width && curHeight == curHeight) {
    ...
    fun testIgnoringBatteryOptimization() {
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                sleep(TIMEOUT_SHORT.toLong())
                val batteryScrollableClazz = scrollableClazz
                ...
                allApps!!.click()
                sleep(TIMEOUT_MEDIUM.toLong())
                val appScrollableClazz = scrollableClazz
    ...
    val guid: Unit
        ...
        get() {
            try {
                ...
                sleep(TIMEOUT_VERY_SHORT.toLong())
                val phxIcon = getUiObject2s("android.widget.ImageView", true, 0.2, 0.5, 0.1, 0.5, 0.2, 0.8, 0.02, 0.5)[0]
                for (i in 0..4) {
                    phxIcon.click()
                }
                ...
                val guid = guidInfo.trim { it <= ' ' }.split("\\s+".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[1]
