...
public class NewWindowMultiPage extends PerCommon {

    private File resultFolder = new File(perFolder, "common");
    private String scenesName = "newwindow-multipage";
    ...
    private void testBrowser(String pkgName, boolean isMem, boolean isFlow) {
        try {
            ...
            urls.add(URL_TIWTTER);
            urls.add(URL_BAIDU);
            for (int i = 0; i < urls.size(); i++) {
                String url = urls.get(i);
                searchOrUrl(pkgName, url, false);
                sleep(10 * 1000);
                if (url.equals(URL_BAIDU) || url.equals(URL_TIWTTER)) {
                    skipOtherDialog();
                }
                if (i != urls.size() - 1) {
                    newTab(pkgName);
