...
public class CurWindowEightPage extends PerCommon {

    private File resultFolder = new File(perFolder, "common");
    private String scenesName = "curwindow-eightpage";
    ...
    private void testBrowser(String pkgName, boolean isMem) {
        try {
            ...
            // 测试场景: 当前窗口连续访问8次google
            for (int i = 0; i < 8; i++) {
                if (i == 0) {
                    searchOrUrl(pkgName, URL_GOOGLE, false);
                } else {
                    searchOrUrl(pkgName, URL_GOOGLE, true);
                }
                sleep(10 * 1000);
            }
